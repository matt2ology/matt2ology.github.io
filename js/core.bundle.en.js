...
function initIndex() {
  ...
  index.add(
      ...
      
      {
        id: 0,
        tag: "en",
        href: "/reference/articles/rdwse---ryan-schachte---creating-a-blog-with-hugo-and-github-in-10-minutes/",
        title: "Creating a Blog With Hugo and Github in 10 Minutes",
        description: "Creating a Blog With Hugo and Github in 10 Minutes \u0026nbsp; published-date: 2020-12-21\nLink: Creating a Blog With Hugo and Github in 10 Minutes\nHighlights \u0026nbsp; id820195256 \u0026nbsp; one repository on github that houses the actual code that we\u0026rsquo;re iterating on uh you know making our posts and our drafts etc and then we\u0026rsquo;re going to have another repository that we\u0026rsquo;re actually deploying off of and this will be hosted through github pages - (View Highlight)\n",
        
        
        content: "Creating a Blog With Hugo and Github in 10 Minutes \u0026nbsp; published-date: 2020-12-21\nLink: Creating a Blog With Hugo and Github in 10 Minutes\nHighlights \u0026nbsp; id820195256 \u0026nbsp; one repository on github that houses the actual code that we\u0026rsquo;re iterating on uh you know making our posts and our drafts etc and then we\u0026rsquo;re going to have another repository that we\u0026rsquo;re actually deploying off of and this will be hosted through github pages - (View Highlight)\nInitial thought or note on: (View Highlight)\nHugo development Repo: a repo that house the Hugo source code and framework GitHub Pages: Another repo that will just house the output \u0026ldquo;pages\u0026rdquo; (i.e. the Markdown to HTML code) id820195555 \u0026nbsp; hugo has some nice themes available on their site again the link - (View Highlight)\nInitial thought or note on: (View Highlight) Hugo Themes: https://themes.gohugo.io/themes/\nid819842801 \u0026nbsp; url that will be kind of the prefix to all of the sub pages that you visit for github - (View Highlight)\nInitial thought or note on: (View Highlight) In config.toml baseURL = https://username.github.io/\nid819842627 \u0026nbsp; we want to check is we want to see if this actually looks okay so we can actually preview this site locally in order to do that what we need to do is we just run hugo server - (View Highlight)\nInitial thought or note on: (View Highlight) hugo server\nid820252861 \u0026nbsp; For GitHub, it\u0026rsquo;s very simple; you\u0026rsquo;re just going to use the name of the project that you\u0026rsquo;re deploying off. - (View Highlight)\nInitial thought or note on: (View Highlight)\nbaseURL = \u0026#34;https://username.github.io/\u0026#34; languageCode =\u0026#34;en-us\u0026#34; title = \u0026#34;My blog\u0026#34; theme =\u0026#34;\u0026lt;theme\u0026gt;\u0026#34; id820253320 \u0026nbsp; run hugo server - (View Highlight)\nInitial thought or note on: (View Highlight) Run locally to see the output\n$ hugo server\nid819843255 \u0026nbsp; add a new post. In order to add a new post, you simply just need to go into hugo new posts and then the name of the post. So, we\u0026rsquo;ll just call it mypost.md - (View Highlight)\nInitial thought or note on: (View Highlight) To create a new post:\n$ hugo new posts/filename.md\nid819843556 \u0026nbsp; deploying off of the main branch - (View Highlight)\nInitial thought or note on: (View Highlight) Main branch of https://username.github.io\nid820251697 \u0026nbsp; we created two repositories; what was the point of that?\u0026quot; So, the point of that is that this is where the code is going to live, but this is not where we want to house the static assets that are going to be used in our other blog. So, what we\u0026rsquo;re going to do is add our other blog as a sub module, and that is going to basically allow us to store just the static assets that were generated in a separate repo. This is a good way to organize your code. - (View Highlight)\nInitial thought or note on: (View Highlight)\nDevelopment branch that was created via: hugo new site websitename The public site (https://username.github.io) From the \u0026ldquo;development repo\u0026rdquo;: git submodule add -b main https://github.com/username/username.github.io public id826778011 \u0026nbsp; add a git sub module and that\u0026rsquo;s basically going to be a reference to this repository and we\u0026rsquo;re just going to really easily deploy out of this - (View Highlight)\nInitial thought or note on: (View Highlight) The Hugo, \u0026ldquo;blog\u0026rdquo;, repository will have the GitHub Pages, username.github.io, repository as the \u0026ldquo;public\u0026rdquo; folder as a submodule\nid826778160 \u0026nbsp; my intentions behind this is the static files that get generated will actually add them into the public folder automatically - (View Highlight)\nInitial thought or note on: (View Highlight) From the Hugo \u0026ldquo;Blog\u0026rdquo; repository every time you enter the command hugo to build your site and \u0026ldquo;transpiles\u0026rdquo; the markdown files publishing the files to the public directory as static HTML files. But do note, and a reminder that, Hugo does not clear the public directory before building your site. Existing files are overwritten, but not deleted. This behavior is intentional to prevent the inadvertent removal of files that you may have added to the public directory after the build. Depending on your needs, you may wish to manually clear the contents of the public directory before every build.\nid826778717 \u0026nbsp; generate my static files and the static files again will go into public so all i need to do to build the code is simply run hugo - (View Highlight)\nid826778885 \u0026nbsp; git remote v the origin is pointing to my simple engineer github repository and that\u0026rsquo;s because we\u0026rsquo;re in the public sub module - (View Highlight)\nInitial thought or note on: (View Highlight) git remote -v In directory path, blog/public, (the submodule) you\u0026rsquo;ll see that the origin is pointing to the username.github.io repository\nid826780133 \u0026nbsp; it will automatically get deployed and the reason it automatically gets deployed is if we go into settings and we scroll down you\u0026rsquo;ll notice that there is a github pages thing and it says your site is ready to be published at this url and the branch that we\u0026rsquo;re deploying off of is the main branch - (View Highlight)"
      })
      ;
